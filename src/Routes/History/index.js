    ...
    axios
      ...
      .then((res) => {
        this.setState({
          ...
          quantity: res.data.length % 10 > 1 ? res.data.length + 10 : 10,
          loading: false,
        });
      });
  }
    ...
    return (
      <Wrapper>
        ...
        {this.state.loading ? (
            ...
            return (
              ...
              >
                <div className={[styles.infos].join(" ")}>
                  <h4>
                    {trans.type} {<small>( {trans.id} )</small>}
                  </h4>
                  <p>{trans.date}</p>
                </div>
                ...
                <div className={[styles.infos].join(" ")}>
                  ...
                  <p
                    ...
                    style={{
                      backgroundColor: this.state[trans.status],
                    }}
                  >
                    {trans.status[0].toUpperCase() + trans.status.slice(1)}
                  </p>
                </div>
              </div>
            );
          ...
          <p style={{ textAlign: "center" }}>No Transaction History</p>
        )}
      </Wrapper>
    );
  }
